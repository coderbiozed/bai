    ...
    <div class="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between fade-up">
        <div class="max-w-2xl">
            ...
            @if ($subcategory->description)
                <p class="mt-4 text-base text-ink/65">{{ $subcategory->description }}</p>
            @endif
        </div>
        @auth
    ...
    <div class="mt-12 space-y-4 fade-up-delay">
               ...
               class="surface block rounded-3xl p-6 transition hover:-translate-y-0.5 hover:border-teal/25">
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    @if ($prompt->is_verified)
                        <span class="rounded-full bg-ink px-3 py-1 text-xs font-semibold uppercase tracking-wider text-paper">Verified</span>
                    @endif
                    @if ($prompt->is_free)
                ...
                <h2 class="mt-3 font-display text-xl font-700 tracking-tight">{{ $prompt->title }}</h2>
                <p class="mt-3 line-clamp-2 text-sm leading-relaxed text-ink/60">{{ $prompt->body }}</p>
                @if ($prompt->tags->isNotEmpty())
                    <div class="mt-4 flex flex-wrap gap-2">
                        @foreach ($prompt->tags as $tag)
                            <span class="text-xs text-ink/40">#{{ $tag->name }}</span>
                        @endforeach
                    </div>
                @endif
            </a>
        @empty
            <div class="surface rounded-3xl p-10 text-center">
                <p class="text-ink/60">No prompts in this specialty yet.</p>
