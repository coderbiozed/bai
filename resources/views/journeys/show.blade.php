...
<section class="shell py-14 sm:py-20">
    <nav class="mb-8 text-sm text-white/55">
        ...
        <span class="mx-2">/</span>
        <span class="text-white">{{ $journey->title }}</span>
    </nav>
    ...
    <div class="grid gap-10 lg:grid-cols-[1.2fr_0.8fr]">
        <div class="fade-up">
            ...
            <h1 class="mt-4 font-display text-4xl font-extrabold tracking-tight text-white sm:text-5xl">{{ $journey->title }}</h1>
                        ...
                        <li>
                            <a href="{{ route('journeys.step', [$journey, $step->step_number]) }}"
                               class="flex gap-3 rounded-2xl px-3 py-3 transition hover:bg-teal/5">
                                <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-lime text-xs font-semibold text-deep">
                                    {{ $step->step_number }}
                                </span>
                                <span>
